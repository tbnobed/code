...
- DGX host fact (confirmed July 2026): Ollama on the user's DGX Spark is the SNAP package (`/snap/bin/ollama`), native process already listening on `*:11434` (why the forge container reaches it). No plain `ollama.service` exists — host env tuning (OLLAMA_CONTEXT_LENGTH/OLLAMA_KEEP_ALIVE) goes in a systemd drop-in on the snap unit — confirmed name `snap.ollama.listener.service` (the snap's app is `listener`, not `ollama`); drop-ins survive snap refreshes. If ever migrating to the official installer: copy models out of /var/snap/ollama/common first and set OLLAMA_HOST=0.0.0.0 (installer unit binds 127.0.0.1 by default). Snap channel may lag official releases — matters because Ollama updates fix tool-call streaming bugs.
